...
export default function RefundBar() {
	...
	const [selected, setSelected] = useState<ListServiceSelectedModel[]>(refundState.tempService);
	...
	const handleSelectEmployees = (employees: any) => {
		setSelectedNumber(null);
		if (refundState.ListServiceEditEmployee) {
			...
			if (employeeExists) {
				...
				const updatedItem = new ListServiceSelectedModel(
					employees,
					refundState.ListServiceEditEmployee.ListService,
					refundState.ListServiceEditEmployee.ListGiftCard,
					refundState.ListServiceEditEmployee._id,
					undefined,
					0,
					refundState.ListServiceEditEmployee.discount,
					refundState.ListServiceEditEmployee.status,
					refundState.ListServiceClick?.OrderDetailId,
					undefined
				);
				setSelected((prevSelected) => {
					return prevSelected.map((item) =>
						item._id === refundState.ListServiceEditEmployee?._id ? updatedItem : item
					);
				});
			...
			if (selected.some((item) => item.Employee?._id === employees?._id)) {
			...
			} else if (refundState.ListServiceClick && refundState.ListServiceClick?.Employee == undefined) {
				setSelected((prevSelected) => {
					const updatedSelected = prevSelected.map((item) => {
						if (item._id === refundState.ListServiceClick?._id) {
							const change = new ListServiceSelectedModel(
								employees,
								item.ListService,
								item.ListGiftCard,
								refundState.ListServiceClick?._id,
								undefined,
								0,
								refundState.ListServiceClick.discount,
								refundState.ListServiceClick.status,
								refundState.ListServiceClick.OrderDetailId,
								undefined
							);
				...
				setSelected((prevSelected) => {
					...
					const selectedOnClickIndex = updatedSelected.findIndex(
						(item) => item === refundState.ListServiceClick
					);

					if (selectedOnClickIndex !== -1 && refundState.ListServiceClick) {
						updatedSelected[selectedOnClickIndex] = new ListServiceSelectedModel(
							employees,
							refundState.ListServiceClick.ListService,
							refundState.ListServiceClick.ListGiftCard,
							uuidv4(),
							undefined,
							0,
							0,
	...
	const menuitem = (item: any) => {
		return (
			<div className="menu-item-2 me-2" style={{ borderRadius: 12 }} onClick={() => setDialogVisible(true)}>
		...
		return (
			<>
				<div className="d-flex align-items-center fs-value fw-normal">
					{t("refund")}&ensp;<span className="fs-title fw-bold">{refundState?.code}</span>&ensp;
					{refundState?.code != undefined && (
						<WtcButton
							label={""}
							...
							height={30}
							onClick={() =>
								handleClickCreateQrCode(refundState.code ?? "", refundState?.customer || undefined)
							}
						/>
					)}
				</div>
	...
	useEffect(() => {
		if (!_.isEqual(refundState.tempService, selected)) {
			setSelected(refundState.tempService);
		}
	}, [refundState.tempService]);

	useEffect(() => {
		!_.isEqual(refundState.tempService, selected);
		{
			dispatch(updatetempService(selected));
			dispatch(calculatorDiscount(refundState.ListDiscount));
		}
	}, [selected]);
	useEffect(() => {
	...
	return (
		<>
			<WtcCard
				borderRadius={12}
				className=""
				title={<>{refundState?.code == undefined ? t("neworder") : headerOrderCode()}</>}
				tools={
