...
type SelectedServiceEmployeeProps = ListServiceSelectedModel & {
	...
	//check open service modal
	isViewSer: boolean;
	setViewSer: (value: boolean) => void;
	setSelectedV2: (value: ListServiceSelectedModel[]) => void;
	selectedV2?: ListServiceSelectedModel[];
};
export default function SelectedServiceEmployee(props: SelectedServiceEmployeeProps) {
	...
	return (
		<>
			<div
				className={`wtc-bg-white item-shadow mb-2 mt-1 d-flex flex-column align-items-center justify-content-between border-active py-2 ${
				...
				<div className="d-flex align-items-center p-0 pt-0 px-2 justify-content-between w-100">
					<span style={fontStyle1} className="d-flex align-items-center flex-grow-1 overflow-hidden">
						{!props.isView && props?.status?.code === StatusValueOrder.inprocessing ? (
							<i
								className="ri-delete-bin-line text-danger fw-normal fs-5"
								onClick={(e) => {
									e.stopPropagation();
														...
														onClick={(e) => {
															...
															warningWithConfirm({
																...
																confirm: () => {
																	...
																	// );
																	//first filter employee
																	const emplIndex = props.selectedV2!.findIndex(
																		(item) =>
																			item.Employee!._id === props.Employee!._id
																	);
																	// Get the employee's service list at the found index
																	const employeeServices =
																		props.selectedV2![emplIndex].ListService;

																	// Remove the service at the specified index
																	const updatedServices = employeeServices!.filter(
																		(_, serviceIndex) => serviceIndex !== index
																	);
																	// Create updated selectedV2 array with the modified services
																	const updatedSelectedV2 = [...props.selectedV2!];
																	updatedSelectedV2[emplIndex] = {
																		...updatedSelectedV2[emplIndex],
																		ListService: updatedServices,
																	};
																	// Update the state using setSelectedV2
																	props.setSelectedV2(updatedSelectedV2);
																	// dispatch(updatetempService(serviceFomrListServcie));
																	// dispatch(
																	// 	calculatorDiscount(newOderState.ListDiscount)
																	// );
																},
															});
														}}
													></i>
													{/* {!props.isView && !(props?.status?.code === "INPROCESSING") && (
														<i
															className="fw-normal ri-delete-bin-line text-muted fs-5 p-2"
															onClick={(e) => {
																e.stopPropagation();
																warningWithConfirm({
																	...
																	confirm: () => {
																		...
																		const employeeIndex = updatedSelected.findIndex(
																			(emp) => emp._id === props._id
																		);

																		if (employeeIndex !== -1) {
																			// Xóa service tại index đã chọn
																			updatedSelected[
																				employeeIndex
