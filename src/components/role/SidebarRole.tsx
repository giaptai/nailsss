...
const SidebarRole = ({
	...
	const getNextRoleCode = (list: RoleModel[]) => {
		const maxCode = Math.max(...list.map((role) => parseInt(role.code.replace("R", ""), 10)));
	...
	useEffect(() => {
		if (roleState.item && action === "UPD") {
			formik.setValues(roleState.item);
		}
	}, [roleState]);
	useEffect(() => {
		if (formik.values !== roleState.item) {
			setIsDisabled(false);
		} else {
			setIsDisabled(true);
		}
	}, [formik.values, roleState.item]);
	useEffect(() => {
		if (roleState.action === "INS" && dialogVisible) {
			formik.setFieldValue("code", getNextRoleCode(roleState.list));
		}
	}, [roleState, dialogVisible]);
				...
				header={
					<>
						<div className="d-flex justify-content-between align-items-center">
							...
							>
								{action === "INS" ? "Add role" : action === "UPD" ? "Edit role" : ""}
							</h2>
						</div>
					</>
				}
				...
				>
					...
					<div className="mb-3 d-flex flex-column" style={{ gap: "4px" }}>
						...
						<div style={{ position: "relative", display: "inline-block", width: "100%" }}>
							<textarea
								placeholder={"Input role's description"}
								maxLength={360}
								className="form-control"
							...
							>
								{formik.values?.note?.length ?? 0}/360
							</span>
						</div>
						{formik.errors.note && <small className="p-error">{formik.errors.note as string}</small>}
					</div>
				</div>
				{action === "INS" ? (
				...
				footer={
					...
					>
						...
						<Button
							...
							onClick={() => {
								setVisibleConfirm(false);
								handleDeleteRole(roleState.item ? roleState.item._id : "");
							}}
							autoFocus
						/>
					</div>
