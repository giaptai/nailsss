...
export const OrderService = {
	convertObjectToServiceByEmployee(
		...
		objectServiceByEmployee.forEach((item) => {
			...
			const totalAmountEmployeePrice = ListService
				? Number(ListService.reduce((sum, payment) => sum + payment.totalPriceEmployee, 0))
					? Number(ListService.reduce((sum, payment) => sum + payment.totalPriceEmployee, 0))
					: 0
				: 0;
	...
	async fetchOrder(data: any) {
		const response = await HttpService.doGetRequest("/orders?from=" + data.fromDate + "&to=" + data.toDate, {});
		const res = parseCommonHttpResult(response);
		return res;
	},
	async deleteOrderCheckIn(id: string) {
		const response = await HttpService.doDeleteRequest("/check-in/" + id, {});
		const res = parseCommonHttpResult(response);
		return res;
	},
	async deleteOrderSoft(id: string) {
		const response = await HttpService.doPatchRequest("/orders/" + id + "/soft-delete", {});
		const res = parseCommonHttpResult(response);
		return res;
	},
	async addOrder(data: any) {
		const response = await HttpService.doPostRequest("/orders", data);
		const res = parseCommonHttpResult(response);
		return res;
	},
	async editOrder(data: any) {
		const response = await HttpService.doPatchRequest("/orders/" + data.id, data.data);
		const res = parseCommonHttpResult(response);
		return res;
	},
	async transferOrder(data: any) {
		const req = { details: data.data };
		const response = await HttpService.doPatchRequest("/orders/" + data._id, req);
		const res = parseCommonHttpResult(response);
		return res;
	},
	async deleteOrderDetail(data: any) {
		const req = { details: data.data };
		const response = await HttpService.doPatchRequest("/orders/" + data._id, req);
		const res = parseCommonHttpResult(response);
		return res;
