...
export const i18Get = (id: string) => t(id) ?? id;
...
export const isStrContainIgnoreCase = (a: string, b: string) => a?.toLowerCase().includes(b?.toLowerCase());
export const createShortName = function (name: string) {
	//console.log(name)
	const shortName = name
		?.toLowerCase()
		.split(" ")
		.map((elem) => elem[0] && elem[0]?.toUpperCase())
		.slice(0, 2)
		.join("");
	return shortName;
};
export const toLocaleStringRefactor = (value: number) => {
	const stringNumber = (value ?? 0).toLocaleString("en-US", { style: "currency", currency: "USD" });
