...
export default function RoleItem() {
	...
	const formikAbility = useFormik<AbilityModel>({
		...
		onSubmit: (data) => {
			const submitData: any = {
				...
				fields: data.permission.name === "all" ? null : data.fields,
			};
	...
	const permissionOptions = selectedMasterPermission
								...
								>
									<WtcDropdownIconText
										...
										options={masterPermissions
											...
											.map((item) => {
												return {
													label: item.name === "User" ? "EMPLOYEE" : item.name.toUpperCase(),
													value: item._id,
												};
											})}
								...
								>
									<WtcDropdownIconText
										...
										onMultiChange={(values) => {
											const selectedValues = values.map((it: any) => it.value);
											if (
												selectedValues.some((value: string) => value !== "read") &&
												!selectedValues.includes("read")
											) {
												selectedValues.push("read");
											}
											formikAbility.setFieldValue("permission.action", selectedValues);
										}}
									/>
								</div>
							</div>
